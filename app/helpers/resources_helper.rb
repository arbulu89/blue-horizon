...
module ResourcesHelper
  ...
  def ssh_cmd(outputs, output_name, index=0)
    ...
    "ssh -o ProxyCommand=\"ssh -W %h:%p #{outputs[:admin_user]}@#{outputs[:bastion_ip]} -i #{private_key} -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no\" #{outputs[:admin_user]}@#{private_address} -i #{private_key} -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no"
  end
